...
;; 
;; # as in Snippets/NSBezierPath (bez).yasnippet
...
;; 
;; # as in Snippets/Method Interface (m).yasnippet
;; ${4/(NS([AEIOQUY])?(\w+).*)|(.)?.*/(?1:a(?2:n$2)$3:(?4:anArgument))/}                      =yyas> (yas-unknown)
;; 
;; # as in Snippets/Class Method (M).yasnippet
;; ${1/^(void|IBAction)$|(.*)/(?2:
...
;; 
;; # as in Snippets/Scalar Accessors Interface (acc).yasnippet
...
;; 
;; # as in Snippets/Write to defaults (setprefs).yasnippet
;;                                                                                            =yyas> (yas-unknown)
;; 
